                            ...
                            <table class="table table-hover table-bordered" id="sampleTable">
                                ...
                                @foreach($video as $vi)
                                    ...
                                    <tr class="tr_with_{{$vi->id}}">
                                        ...
                                        <td class="hidden-sm hidden-xs"> {{ $vi->video_category->name }} </td>
                                        <td class="hidden-sm hidden-xs"> {{ $vi->filename }} </td>
                                        <td class="hidden-sm hidden-xs"> {{ $vi->created_at }} </td>
                                        <td style="text-align:center">

                                            <a href="{{route('videomanager.edit',$vi->id)}}" class="btn-primary btn-block" > Edit</a>

                                            <form action="{{route('videomanager.destroy',$vi->id)}}" id="form_delete" accept-charset="UTF-8" method="POST" style="margin-top:3px">
                                                {{ csrf_field() }}
    ...
    <script>
        ...
        function ondelete(id){
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        @if($video->count()) @foreach ($video as $v) url: "{{route('videomanager.destroy',$v->id)}}", @endforeach @endif
                        ...
                        success: function (msg) {
                            if (msg.status === 'success') {
                                ...
                                swal("Success!", "Video u fshi me sukses.", "success");
                            }
